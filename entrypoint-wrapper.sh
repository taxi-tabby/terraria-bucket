...
seed_world_from_preload() {
    # On first start (volume has no .wld), copy each .wld and matching .twld
    # bundled at /preload/Map/ into $WORLDS_DIR. Never overwrites: if any .wld
    # is already present in the volume, leave it alone so in-progress games
    # persist across redeploys.
    #
    # When exactly one .wld is seeded, point WORLD_NAME at it so the server
    # actually opens that world instead of autocreating a new one.
    ...
    local src="${PRELOAD_DIR}/Map"
    if [[ ! -d "$src" ]]; then
        echo "[entrypoint] no preload map dir at $src; skipping world seed"
        return 0
    fi
    ...
    if (( ${#wlds[@]} == 0 )); then
        echo "[entrypoint] no .wld in $src; skipping world seed"
        return 0
    fi
    ...
    for wld in "${wlds[@]}"; do
        ...
        echo "[entrypoint] seeded $(basename "$wld") into $WORLDS_DIR"
    done

    if (( ${#wlds[@]} == 1 )); then
        local seeded_name
        seeded_name=$(basename "${wlds[0]}" .wld)
        if [[ "$seeded_name" != "$WORLD_NAME" ]]; then
            echo "[entrypoint] updating WORLD_NAME: '$WORLD_NAME' -> '$seeded_name'"
