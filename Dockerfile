...
RUN apk update \
    && apk add --no-cache bash curl jq nano file libgcc libstdc++ icu-libs unzip \
    && rm -rf /var/cache/apk/*
...
# /preload contains only small text/zip seed data:
#   - mods.json  — single source of truth for which .tmod files to install
#                  (entrypoint fetches the actual binaries from GitHub Releases
#                   on first start, verifying sha256 from this manifest)
#   - Map/Terraria.zip — optional starter world (extracted on first start
#                        if the volume has no .wld)
